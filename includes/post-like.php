...
function like_scripts() {
	wp_enqueue_script( 'jm_like_post', get_template_directory_uri().'/scripts/post-like.js', array('jquery'), '1.0', 1 );
...
function getPostLikeLink( $post_id ) {
	...
	if ( empty( $like_count ) || $like_count == "0" ) { $wrapstart = '<span class="nolikes">'; $wrapend = '</span>'; } else { $wrapstart = ''; $wrapend = ''; }
	if ( AlreadyLiked( $post_id ) ) {
		$class = __(  ' liked' );
		$title = __( 'Unlike' );
		$heart = '<span class="favorite-icon"></span>';
	} else {
		$class = __( '' );
		$title = __( 'Like' );
		$heart = '<span class="favorite-icon"></span>';
	}
	$output = '<a href="#" class="jm-post-like'.esc_attr( $class ).'" data-post_id="'.esc_attr( $post_id ).'" title="'.esc_attr( $title ).'">'.$wrapstart.$count.$heart.$wrapend.'</a>';
	return $output;
}
